...
import { NewsApiResponse, ParamsType, СategoriesApiResponse } from "../interfaces";
...
export const getCatagories = async (): Promise<СategoriesApiResponse> => {
  try {
    const res = await axios.get<СategoriesApiResponse>(`${BASE_URL}available/categories`, {
      params: {
        apiKey: API_KEY,
      },
    });
